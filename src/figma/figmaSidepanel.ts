...
type FigmaSidePanel = {
	figmaDataTab?: SidePanelTab,
	figmaListTab?: SidePanelTab,
	figmaConfigTab?: SidePanelTab,
	figmaDocsListRefreshTimer?: number,
	figmaUrlPathHandlers?: Map<string, FigmaHandlerFunction>,
	filter?: string

	childNodeHandlers?: Map<figma.Node['type'], (docId: string, docName: string, node: any) => HTMLElement>
}


const figmaSidePanel: FigmaSidePanel = {}
...
export function InitFigmaSidepanel() {
	...
	function handleFigmentMessage(message: Message) {
		switch (message.action) {
			case 'search_figma_data':
				...
				figmaSidePanel.filter = message.str
				const filterInput = document.querySelector('input[placeholder=filter]') as HTMLInputElement
				if (filterInput) {
					filterInput.value = figmaSidePanel.filter
				}
				if (file) {
					filterFigmaData(file, figmaSidePanel.filter)
				}
	...
	figmaSidePanel.figmaUrlPathHandlers = new Map<string, FigmaHandlerFunction>()
	figmaSidePanel.figmaUrlPathHandlers.set('file', ({ params: [docId] }: FigmaHandlerParams) => handleFigmaFileURL(docId)) //handle https://www.figma.com/file....
	figmaSidePanel.figmaUrlPathHandlers.set('design', ({ params: [docId] }: FigmaHandlerParams) => handleFigmaFileURL(docId)) //handle https://www.figma.com/file....
	figmaSidePanel.figmaUrlPathHandlers.set('*', ({ path }) => displayStatus(`Open a specific figma file to read it`, 'figma'))

	figmaSidePanel.childNodeHandlers = new Map<figma.Node['type'], (docId: string, docName: string, node: any) => HTMLElement>()
	figmaSidePanel.childNodeHandlers.set('CANVAS', renderFigmaCanvasNode)
	figmaSidePanel.childNodeHandlers.set('FRAME', renderFigmaFrameNode)
	figmaSidePanel.childNodeHandlers.set('SECTION', renderFigmaSectionNode)
	...
	figmaSidePanel.figmaDocsListRefreshTimer = undefined

	figmaSidePanel.figmaDataTab = new SidePanelTab(figmaTabTitle('Localhost'), renderFigmaDataUi())
	figmaSidePanel.figmaListTab = new SidePanelTab(figmaTabTitle('Docs'), renderFigmaDocsList())
	figmaSidePanel.figmaConfigTab = new SidePanelTab(figmaTabTitle('Config'), renderFigmaConfigUi())
}
...
export const handleFigmaUrl: sidePanelUrlHandler = function (url: URL) {
	...
	const handler = figmaSidePanel.figmaUrlPathHandlers?.get(path) ?? figmaSidePanel.figmaUrlPathHandlers?.get('*')
...
async function handleFigmaFileURL(docId: string) {
	...
	if (!userToken) {
		...
		return figmaSidePanel.figmaConfigTab?.setActive()
	}
	...
	figmaSidePanel.figmaListTab?.setTabBody(renderFigmaDocsList())
	figmaSidePanel.figmaListTab?.setActive()
}
...
export const handleLocalhost: sidePanelUrlHandler = function (url: URL) {
	...
	figmaSidePanel.figmaDataTab?.setTabBody(figmaDataUi)
	figmaSidePanel.figmaDataTab?.setActive()
}
...
function renderFigmaDocsList() {
	...
	if (figmaSidePanel.figmaDocsListRefreshTimer) clearInterval(figmaSidePanel.figmaDocsListRefreshTimer)
	figmaSidePanel.figmaDocsListRefreshTimer = setInterval(() => {
...
function renderFigmaDataUi() {
	...
	header.appendChild(element('input', {
		type: 'text', placeholder: 'filter', value: figmaSidePanel.filter ?? '',
		onkeyup: (e) => {
			if (filterTimeout) clearTimeout(filterTimeout)
			filterTimeout = setTimeout(() => {
				figmaSidePanel.filter = (e.target as HTMLInputElement)?.value
				for (const file of files) {
					if (file.document)
						filterFigmaData(file.document, figmaSidePanel.filter)
				}
			}, 400)
		}
	}))
...
function renderChildNodes(docId: string, docName: string, node: figma.DocumentNode | figma.CanvasNode | figma.SectionNode) {
	...
	node?.children?.forEach(figmaNode => {
		const handler = figmaSidePanel.childNodeHandlers?.get(figmaNode.type)
